                ...
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="POST" action="{{ route('users.update',$user->id)}}">
                    ...
                    <div class="form-group">
                        ...
                        <select class="select2" style="width:100%" name="role_id">
                          @foreach ($roles as $value)
                            <option value="{{old('role_id',$value->id)}}" @if(old('role_id',$value->id) == $user->role_id) selected @endif>{{$value->name}}</option>
                          @endforeach
                        </select>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputUsername1" placeholder="Name" name="name" value="{{old('name',$user->name)}}">
                      @error('name')
                            <label class="error text-danger">{{$message}}</label>
                        @enderror
                    </div>
